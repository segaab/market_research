...
ProfitableSeasonalMap: Dict[str, Dict] = {
    "Indices": {
        ...
        "DAX": {  # not used but kept for completeness
            "Jan":"⚪","Feb":"⚪","Mar":"⚪","Apr":"✅","May":"⚪","Jun":"⚪",
            "Jul":"⚪","Aug":"⚪","Sep":"❌","Oct":"✅","Nov":"✅","Dec":"✅"
        }
    },
...
# ── HELPER: ENSURE TIMESTAMP ────────────────────────────────────────────────
def ensure_timestamp(df: pd.DataFrame, date_col: str = "date") -> pd.DataFrame:
    """
    Ensures the dataframe has a 'timestamp' column for consistent merging.
    Converts the specified date column to datetime if needed.
    """
    if df.empty:
        return df
...
def fetch_price_history(ticker: str) -> pd.DataFrame:
    ...
    df = ensure_timestamp(df, date_col="date")
    return df
...
def fetch_cot_data(cot_name: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    df = ensure_timestamp(df, date_col="report_date_as_yyyy_mm_dd")
...
with st.spinner("Downloading & crunching data …"):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as pool:
        ...
        # Ensure timestamps
        for t in price_data:
            price_data[t] = ensure_timestamp(price_data[t], "date")

    # COT – sequential
    cot_data = {t: fetch_cot_data(COT_ASSET_NAMES.get(t, ""), START_DATE, END_DATE)
                for t in tickers}
    for t in cot_data:
        cot_data[t] = ensure_timestamp(cot_data[t], "report_date_as_yyyy_mm_dd")
...
# ── VISUALISATIONS ───────────────────────────────────────────────────────────
# ... rest of your code is preserved as-is
